...
public class ScoreChart extends Activity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    Bitmap emptyBmap = Bitmap.createBitmap(400, 300, Config.ARGB_8888);
  ...
  private Bitmap drawPlayerChart(Bitmap bitmap) {
    ...
    cur_elt_array[0] = String.format(mResources.getString(R.string.score_board), mGameManager.getGame()
    ...
    for (Player player : mGameManager.getGame().getPlayerList()) {
      ...
      drawPlayerScores(canvas, player, labels, 0, colors[player.getId()]);
    }
  ...
  private void drawGrid(Canvas canvas, List<String[]> labels) {
    ...
    int p_height = canvas.getHeight() - 10;
    int p_width = canvas.getWidth() - 10;
    int nbCells = mGameManager.getGame().getRoundNumber() + 1;
    int[] tmp_draw_sizes = { 2 + left_margin_d, 25, p_width - 2 - left_margin_d, p_height - 25 - nbCells };
    drawSizes = tmp_draw_sizes; // keep it for later processing

//    drawingBox = new DrawingBox(bottom, top, left, right);
    drawingBox = new DrawingBox(drawSizes[1] + drawSizes[3], drawSizes[1], drawSizes[0], drawSizes[0] + drawSizes[2]);
...
//    canvas.drawRect(drawSizes[0], drawSizes[1], drawSizes[0] + drawSizes[2], drawSizes[1] + drawSizes[3], paint);
    ...
    canvas.drawRect(drawingBox.mChartLeft, drawingBox.mChartTop, drawingBox.mChartRight, drawingBox.mChartBottom, paint);
...
//      canvas.drawLine(drawSizes[0], drawSizes[1] + (i * drawSizes[3] / nbCells), drawSizes[0] + drawSizes[2],
//          drawSizes[1] + (i * drawSizes[3] / nbCells), paint);
//      canvas.drawLine(drawSizes[0] + (i * drawSizes[2] / nbCells), drawSizes[1], drawSizes[0] +
//          (i * drawSizes[2] / nbCells), drawSizes[1] + drawSizes[3], paint);
      // Horizontal lines
      final int horizontalY = drawSizes[1] + (i * drawSizes[3] / nbCells);
      canvas.drawLine(drawingBox.mChartLeft, horizontalY, drawingBox.mChartRight, horizontalY, paint);
      // Vertical lines
      final int vertivalX = drawSizes[0] + (i * drawSizes[2] / nbCells);
      canvas.drawLine(vertivalX, drawingBox.mChartTop, vertivalX, drawingBox.mChartBottom, paint);

    }

    // good for one value
    printAxisLegend(canvas, Double.valueOf(rounded_max).intValue(), Double.valueOf(rounded_min).intValue(),
        cur_elt_array[0], 2, 0);

  }

  private void printAxisLegend(Canvas thisDrawingArea, Integer curMax, Integer curMin, String cur_label, int x_guide,
      int this_idx) {
    int nbYSectors = curMax / 50;
    int nbXSectors = mGameManager.getGame().getRoundNumber() + 1;

    if (nbYSectors == 0 || nbYSectors > 4) {
      // Default value: 4
      nbYSectors = 4;
    }

    Integer delta = (curMax - curMin) / nbYSectors;
    final Paint paint = new Paint();
    ...
    displayYLegend(thisDrawingArea, curMin, x_guide, nbYSectors, delta, paint);
    displayXLegend(thisDrawingArea, curMin, x_guide, nbXSectors, delta, paint);

    paint.setTextSize(10);
//    switch (this_idx) {
//    case 0:
//      thisDrawingArea.drawText(cur_label, x_guide - 2, drawSizes[1] - 15, paint);
//      break;
//    case 1:
//
//      thisDrawingArea.drawText(cur_label, x_guide - 2 - 30, drawSizes[1] - 15, paint);
//      break;
//    }
   ...
   * @param paint
   */
  private void displayYLegend(Canvas thisDrawingArea, Integer curMin, int x_guide, int nbYSectors, Integer delta,
      final Paint paint) {
    Integer lineLabel;
    // '<=' because we display from zero to curMax
    for (int i = 0; i <= nbYSectors; i++) {
      // 'work our the values so is proper

      lineLabel = curMin + delta * i;

      if (i == nbYSectors) {
        thisDrawingArea.drawText(lineLabel.toString(), x_guide - 2, drawingBox.mChartBottom -
            (i * drawSizes[3] / nbYSectors), paint);
      }
      else {
        thisDrawingArea.drawText(lineLabel.toString(), x_guide - 2, drawingBox.mChartBottom -
            (i * drawSizes[3] / nbYSectors) - 3, paint);
      }
    }
  }
   ...
   * @param paint
   */
  private void displayXLegend(Canvas thisDrawingArea, Integer curMin, int x_guide, int nbXSectors, Integer delta,
      final Paint paint) {
    Integer lineLabel;
    // '<=' because we display from zero to curMax
    for (int i = 0; i <= nbXSectors; i++) {
      // 'work our the values so is proper
      lineLabel = i;
//      thisDrawingArea.drawText(lineLabel.toString(), drawSizes[0] + (i * drawSizes[2] / nbXSectors), drawSizes[1], paint);
      thisDrawingArea.drawText(lineLabel.toString(), drawingBox.mChartLeft + (i * drawSizes[2] / nbXSectors), drawingBox.mChartBottom, paint);
    }
  }

  private Point scalePoint(int this_x, double this_y, Point drawPoint, int scr_x, int scr_y, int scr_width,
  ...
  private boolean drawPlayerScores(Canvas canvas, Player player, List<String[]> these_labels, int only_this_idx,
      int lineColor) {
    int lRow;
    int nParms;
    ...
    try // catch in this block for some thing
    {
      ...
      nParms = only_this_idx;
      {
        ...
        // Draw origin point
        point = scalePoint(0, 0, point, drawSizes[0], drawSizes[1], drawSizes[2], drawSizes[3], cur_maxX, cur_minX,
            cur_maxY, cur_minY);
        ...
        if (cur_points_2_plot < POINTS_TO_CHANGE)
          canvas.drawRect(cur_x - 2, cur_y - 2, cur_x - 2 + 4, cur_y - 2 + 4, paint);
        ...
        Set<Entry<Integer, List<Integer>>> entrySet = playerScorePerRound.entrySet();
        ...
        for (Entry<Integer, List<Integer>> entry : entrySet) {
          ...
          for (lRow = 0; lRow < scores.size(); lRow++) {
            ...
            if (cur_OBD_val == Double.NaN) {
              ...
              point = scalePoint(roundNumber, cur_OBD_val, point, drawSizes[0], drawSizes[1], drawSizes[2],
                  drawSizes[3], cur_maxX, cur_minX, cur_maxY, cur_minY);
  ...
  private class DrawingBox {
    ...
    public DrawingBox(int chartBottom, int chartTop, int chartLeft, int chartRight) {
