...
public class Game {
  ...
  /**
   * Add the penalty to the list of player's penalties. Commit penalty score to
   * player's total.
   *
   * @param penalty
   */
  ...
  public Integer getBestRoundScore(Player mPlayer) {
    ...
    Set<Entry<Integer, Integer>> entrySet = getPlayerRoundsScore(mPlayer).entrySet();
  ...
  public Player getMaxHit(Player player) {
    PlayerScore playerScore = player.getPlayerScore();
  ...
  /**
   * Returns the list of player who hit given player.
   *
   * @param player
   * @return
   */
  public List<Player> getMaxHitVictim(Player player) {
